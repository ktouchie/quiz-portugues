...
/**
 * Advanced setup for Vocabulary (docs/MOBILE_APP_SPEC.md §8, GitHub #34): pick categories, then
 * start an uncapped session over just that selection — unlike Quick Practice, not restricted to
 * unlocked CEFR tiers.
 */
@Composable
fun VocabularySetupScreen(
    onStart: (selectedCategories: List<String>) -> Unit,
    viewModel: VocabularySetupViewModel = viewModel(),
) {
